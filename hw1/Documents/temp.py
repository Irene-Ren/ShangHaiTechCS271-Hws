...
if __name__ == "__main__":
    ...
    # model = model.sample_points_uniformly(number_of_points=1000)
    # matrix = np.asarray(model.points, dtype = 'float')

    # ptcloud = o3d.geometry.PointCloud()
    # ptcloud.points = o3d.utility.Vector3dVector(np.random.randn(50,3))
    # matrix = np.asarray(ptcloud.points, dtype = 'double')

    numModels = int(raw_input())
    Hulls = []
    for i in range(numModels):
        ...
        convexHull = convexHull.ExtendConvexHull()
    ...
    for i in range(numModels):
        ...
        HullsGFX.append(pcd)
    o3d.visualization.draw_geometries_with_editing(HullsGFX)
